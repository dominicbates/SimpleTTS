...
class JennyDataset(Dataset):
    ...
    max_frames : int, optional
        If set, samples with more than this many mel frames are dropped.
        Defaults to 1400.
    """
    # def __init__(self, root_dir: str | Path, max_frames: Optional[int] = 1400):
    ...
    #         self.files = self._filter_by_length(all_files)
    #     else:
    #         self.files = all_files

    #     print(f"[JennyDataset] {len(self.files)}/{len(all_files)} samples kept "
    #           f"(max_frames={self.max_frames}) in {self.samples_dir}")

    def __init__(self, root_dir: str | Path, max_frames: Optional[int] = 1400, max_tokens: Optional[int] = 200):
        self.samples_dir = Path(root_dir) / "samples"
        self.max_frames  = max_frames
        ...
        self.files = self._filter(all_files)
        print(f"[JennyDataset] {len(self.files)}/{len(all_files)} samples kept "
              f"(max_frames={self.max_frames}, max_tokens={self.max_tokens}) in {self.samples_dir}")
    
    def _filter(self, all_files: list) -> list:
        kept = []
        for path in all_files:
            sample = torch.load(path, weights_only=True)
            if self.max_frames is not None and sample["mel"].shape[-1] > self.max_frames:
                continue
            if self.max_tokens is not None and len(phonemize(sample["text"])) > self.max_tokens:
                continue
            kept.append(path)
        return kept

    # def _filter_by_length(self, all_files: list) -> list:
    #     """Load each file once to check its frame count, drop if over max_frames."""
    #     kept = []
    #     for path in all_files:
    #         sample = torch.load(path, weights_only=True)
    #         if sample["mel"].shape[-1] <= self.max_frames:
    #             kept.append(path)
    #     return kept
    ...
    def __getitem__(self, i: int) -> tuple[Tensor, str]:
        ...
        mel: Tensor = sample["mel"].float()  # stored as float16, cast to float32
